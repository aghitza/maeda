...
\begin{itemize}
  ...
  \item If $p$ is a prime of type II or III, $F_p$ is a product of distinct
    irreducible factors.  Each of the factors is therefore separable, and they 
...
\begin{proof}
  ...
  into disjoint cycles contains exactly one even cycle $\sigma_0$ (of length
  ...
  cycle $\rho_0$ of prime length $p>d/2$.  Therefore the other cycles have
  ...
  and it follows that $n|d$. Note that $n>1$ since $G$ contains at least one
  transposition, namely $\tau_1^a$. 
  Let $T=\{e_1,e_2,\ldots,e_p\}$ be the subset of $S$ permuted by
  ...
  and $m\mid p$. Since $n>1$, we have $m>1$, so $m=p$ since $p$ is prime.
...
\begin{proof}
  ...
  \begin{equation*}
    ...
    e^{\frac{1}{24(c-2)+1}-\frac{1}{6c-12}}
    >\frac{1}{3\sqrt{\pi(c-2)}}>\frac{\sqrt{2}}{3\sqrt{\pi}}\,\frac{1}{\sqrt{d}}
    >\frac{1}{4\sqrt{d}},
  \end{equation*}
  where we use $\sqrt{2}/3\sqrt{\pi}\approx 0.266$ and the elementary inequalities
  \begin{align*}
    e^{\frac{1}{24(c-2)+1}-\frac{1}{6c-12}}>e^{-\frac{1}{8(c-3)}}&>\frac{4}{5}\text{ if }c\geq 4,\\
    \frac{2c-3}{2c-2}&\geq \frac{5}{6}\text{ if }c\geq 4.
  \end{align*}
  This gives us the desired lower bound for $d\geq 8$, and the remaining
  cases $2<d<8$ are easily checked.
\end{proof}
