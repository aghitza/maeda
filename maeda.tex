...
\begin{proof}
  ...
  Let $K/\QQ$ be the splitting field of $F$. Let $l$, $q$ and $r$ be the primes of
  $F$ of types $I$, $II$ and $III$, respectively. Let $G=\text{Gal}(K/\QQ)$. Thus
  ...
  TODO: Distinct roots mod $q$ implies Frob$_\mathcal{p} \in G$ for $\mathcal{p}$
  a prime of $K$ above $p$.
  TODO: Frob$_\mathcal{p}$ has cycle decomposition which is the same as the
  decomposition of $F_p$.
  So Frob$_\mathcal{q}$ has cycle type $2,\deg(f_1),\ldots,\deg(f_{t_1})$ and
  Frob$_\mathcal{r}$ has cycle type $p,\deg(g_1),\ldots,\deg(g_{t_2})$, where 
  $p>d/2$ is prime. Thus for $x_1=\prod_{i=1}^{t_1}\deg f_i$ and $x_2=\prod_{i=1}^{t_2}\deg g_i$,
  we have that (Frob$_\mathcal{q})^{x_1}$ is a transposition and (Frob$_\mathcal{r})^{x_2}$
  ...
  transposition $(a~b)\in G$. This is an equivalence relation on $S$. Since $G$ is
  ...
  (Frob$_\mathcal{r})^{x_2}$, and let $G_T$ be the subgroup of $G$ fixing $S\setminus T$.
  ...
  say $m$, and $m|p$. Since $n>1$, we have $m>1$, so $m=p$ since $p$ is prime.
